...
self.hasNulls = (values) => {
  ...
  values.forEach((value) => {
    if (this.isType(value, 'array')) {
      nulls = this.hasNulls(value);
    }
    else {
      if (value === null) {
        nulls = true;
...
self.filterNulls = (values) => {
  if (!this.isType(values, 'array')) {
    return values;
  }
  return values.map((value) => {
    if (this.isType(value, 'array')) {
      return this.filterNulls(value)
    }
    return value;
  }).filter((value) => {
    return value !== null;
...
self.validateDate = (date, format) => {
  switch (format) {
    ...
    case 'timestamp':
      return this.isUnixTimestamp(date);
    default:
...
self.getType = (value) => {
  ...
  for (let key in types) {
    if (this.isType(value, types[key])) {
      return types[key];
    }
  }
  return false;
