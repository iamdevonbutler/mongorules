...
class Validator {
  ...
  validateType(type) {
    ...
    if (type === 'date') {
      if (!utils.validateDate(this.value, this.schema.dateFormat)) {
        this.addError('type', { expected: type + ': ' + this.schema.dateFormat });
      }
    }
    else {
      if (!utils.isType(this.value, type)) {
  ...
  validateDenyXSS() {
    ...
    if (utils.isType(this.value, 'string') && this.value !== sanitize.inHTMLData(this.value)) {
