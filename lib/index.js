...
var proxy = {
   ...
   * @tests none
   */
  _handleErrors(errors, collectionName, action, localErrorHandler, globalErrorHandler) {
    debug(`Errors: collection "${collectionName}", action "${action}"` , JSON.stringify(errors, null, 2));
    if (localErrorHandler) {
      localErrorHandler.onError.call(null, collectionName, action, errors);
    }
    if (globalErrorHandler) {
      globalErrorHandler.call(null, collectionName, action, errors, !!localErrorHandler);
    }
    // Default handler.
    if (!localErrorHandler && !globalErrorHandler) {
      if (errors.name === 'DocumentValidationError') {
        throw new DocumentValidationError(collectionName, action, errors);
      }
  ...
  _mongodbApplyHandler: {
    apply(target, thisArgument, argumentsList) {
      var state, _this, collection, model, preprocessOperation, globalErrorHandler, localErrorHandler;
      _this = proxy;
      ...
      localErrorHandler = model.onError;

      // If we are preprocessing a insert, or update operation...
      preprocessOperation = ['insert', 'update', 'findAndModify'].indexOf(state.action) > -1;
      if (model && model.schema && !state.novalidate && preprocessOperation) {
        ...
        // Handle errors.
        if (result.errors) {
          _this._handleErrors(result.errors, state.collectionName, state.action, localErrorHandler, globalErrorHandler);
      ...
      collection = mongorules.getCurrentDatabase().collection(state.collectionName);
      ...
      return new Promise((resolve, reject) => {
        ...
        argumentsList.push((err, result) => {
          if (err) {
            _this._handleErrors(err, state.collectionName, state.action, localErrorHandler, globalErrorHandler);
            reject(err);
          }
          resolve(result);
        });
