...
var proxy = {
  ...
  _preprocessQuery(argumentsList, model, collectionName, action) {
    ...
    return !result.errors ? argumentsList : false;
  },
  ...
  _mongodbApplyHandler: {
    apply(target, thisArgument, argumentsList) {
      ...
      if (model && !state.novalidate) {
        let result;
        // Only preprocess insert, update, and save.
        if (state.action === 'insert' || state.action === 'update' || state.action === 'save') {
        ...
        // If this was a insert, update, save, and it retured validated,
        // transformed document successfully.
        if (result) {
