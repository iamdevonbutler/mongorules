...
var proxy = {
  ...
  _preprocessQuery(argumentsList, model, collectionName, action) {
    var result, data, globalErrorHandler;

    // Validate data.
    switch (action) {
      case 'insert':
        data = argumentsList[0];
        errors = validate._validateInsert(data, model.schema)
      case 'update':
        data = argumentsList[1];
        errors = validate._validateUpdate(data, model.schema)
      case 'save':
        data = argumentsList[0];
        errors = validate._validateSave(data, model.schema)
    }
    ...
    if (errors) {
      ...
      if (model.onError) {
        model.onError.call(null, collectionName, action, errors);
        return false;
      }
      // Global error handler.
      else if (globalErrorHandler) {
        globalErrorHandler.call(mongoproxy, collectionName, action, errors);
        return false;
      }
      // Custom error handler.
      else {
        throw new FieldValidationError(collectionName, action, errors);
      }
    }

    // Transform data (I hate doing stuff by reference).
    transform._transformData(data, schema);
