...
var proxy = {
   ...
   * @tests none
   */
  _handleErrors(err, type, collectionName, action, locaHandler, globalHandler) {
    debug(`Errors: collection "${collectionName}", action "${action}"` , JSON.stringify(err, null, 2));
    if (locaHandler) {
      locaHandler.onError.call(null, collectionName, action, err);
    }
    if (globalHandler) {
      globalHandler.call(null, collectionName, action, err, !!locaHandler);
    }
    // Default handler.
    if (!locaHandler && !globalHandler) {
    ...
    apply(target, thisArgument, argumentsList) {
      ...
      return new Promise((resolve, reject) => {
        ...
        argumentsList.push((err, result) => {
          if (err) {
            _this._handleErrors(err, 'MongodbError', state.collectionName, state.action, localErrorHandler, globalErrorHandler);
            reject(err);
          }
          resolve(result);
        });
