...
module.exports = {
  ...
  close() {
    ...
    if (!db) throw '>>> cannot close connection - no databases yet exist.';
    return new Promise((resolve, reject) => {
      db.close((err, result) => {
        if (err) {
          debug('Error closing to mongo connnection.');
          reject(err);
        }
        debug('Connected to mongo sucessfully closed.');
        resolve(result);
      });
    });
  },
   ...
   */
  _getDatabase() {
    return Object.keys(this._db).length ? this._db : null;
  },
