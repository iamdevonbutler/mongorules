...
class DocumentValidationError extends Error {
  constructor(collectionName, operation, errors) {
    super();
    this.name = 'Mongorules - document validation error';
    this.message = `Operation "${operation}" on collection "${collectionName}" failed validation.`;
    this.errors = errors;
  }
}
 ...
 * @tests none
 */
var handleErrors = (err, type, collectionName, operation, localHandler, globalHandler) => {
  if (localHandler) {
