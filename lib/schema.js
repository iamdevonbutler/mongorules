...
module.exports = {
  ...
  _preprocessSchema(schema, isChildSchema = false) {
    ...
    // Add _id field w/ validate function to top level schema.
    if (!isChildSchema) {
    ...
    for (let fieldKey in schema) {
      ...
      if (utils._isType(fieldValue, 'array')) {
        let childObjectsInArray, childObjectFields, type, properties;
        ...
        if (childObjectsInArray) {
          // Remove child object fields from schema to prevent double processing.
          Object.keys(childObjectsInArray).forEach((childFieldKey) => {
            delete schema[childFieldKey];
          });
          // If array of arrays.
          if (fieldValue[0][0]) {
            type = 'arrayofarraysofobjects';
            properties = fieldValue[0][0];
          }
          // If array of objects.
          else {
            type = 'arrayofobjects';
            properties = fieldValue[0];
          }
        }
        // Is array of values.
        else {
        ...
        this._setSchemaDefaults(properties);
        this._arrayifySchemaValue(properties);
        this._validateSchema(properties, fieldKey);
        validationObject[fieldKey] = properties;
        if (childObjectsInArray) {
          validationObject[fieldKey]._schema = this._preprocessSchema(childObjectsInArray, true);
        }
      }
      // If the field is not an array but an value in the document.
      else {
        fieldValue._type = 'values';
