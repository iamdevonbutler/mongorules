...
self.preprocessSchema = (schema) => {
  ...
  for (let fieldKey in schema) {
    ...
    if (utils.isType(fieldValue, 'array')) {
...
self._validateSchemaField = (field, schemaName) => {
  ...
  if (field.type && !utils.isType(field.type, 'string'))
  ...
  if ( (field.validate[0] && !utils.isType(field.validate[0], 'function')) || (field.validate[1] && !utils.isType(field.validate[1], 'function')) )
    throw new SchemaValidationError(schemaName, '`validate` must be a function or array containing a function(s).');

  if ( (field.transform[0] && !utils.isType(field.transform[0], 'function')) || (field.transform[1] && !utils.isType(field.transform[1], 'function')) )
    throw new SchemaValidationError(schemaName, '`transform` must be a function or array containing a function(s).');

  if (field.dateFormat && !utils.isType(field.dateFormat, 'string'))
    throw new SchemaValidationError(schemaName, '`dateFormat` must be a string.');

  if (field.filterNulls !== null && !utils.isType(field.filterNulls, 'boolean'))
    throw new SchemaValidationError(schemaName, '`filterNulls` must be a boolean.');

  if (!utils.isType(field.minLength, 'number') && !utils.isType(field.minLength, 'array'))
    throw new SchemaValidationError(schemaName, '`minLength` must be a number or an array of numbers.');

  if (!utils.isType(field.maxLength, 'number') && !utils.isType(field.maxLength, 'array'))
...
self._arrayifySchemaField = (field) => {
  if (!utils.isType(field.minLength, 'array')) {
    field.minLength = [field.minLength, null];
  }

  if (!utils.isType(field.maxLength, 'array')) {
    field.maxLength = [field.maxLength, null];
  }

  if (!utils.isType(field.validate, 'array')) {
    field.validate = [field.validate, null];
  }

  if (!utils.isType(field.transform, 'array')) {
