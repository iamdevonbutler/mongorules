...
module.exports = {
  ...
  _validateSchema(schema, schemaName) {
    ...
    if ( schema.notNull[0] !== null && !_.isBoolean(schema.notNull[0]) || (schema.notNull[1] !== null && !_.isBoolean(schema.notNull[1])) )
  ...
  _makeSchemaValuesArrays(schema) {
    ...
    if (!utils._isType(schema.notNull, 'array')) {
      schema.notNull = [schema.notNull];
    }
