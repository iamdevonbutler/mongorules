...
module.exports = {
  ...
  _getArrayObjectChildren(schema, fieldKey) {
    ...
    for (let schemaKey in schema) {
      // Split strings on `.` to find children nested only one level.
      let fieldSplit, schemaSplit;
      fieldSplit = fieldKey.split('.');
      schemaSplit = schemaKey.split('.');
      if (schemaKey.indexOf(fieldKey + '.') > -1) {
        let newSchemaKey = schemaSplit.splice(fieldSplit.length).join('.');
        arrayObjects[newSchemaKey] = schema[schemaKey];
      }
    }
      ...
      if (utils._isType(field, 'array')) {
        ...
        if (arrayObjectChildren) {
          ...
          Object.keys(arrayObjectChildren).forEach((childFieldKey) => {
            delete schema[fieldKey + '.' + childFieldKey];
          });
          // If array of arrays.
          if (field[0][0]) {
  ...
  _validateSchema(schema, schemaName) {
    ...
    if (schema.minLength !== null && !_.isNumber(schema.minLength))
      throw new SchemaValidationError(schemaName, '`maxLength` must be a number.');
    if (schema.maxLength !== null && !_.isNumber(schema.minLength))
      throw new SchemaValidationError(schemaName, '`maxLength` must be a number.');
