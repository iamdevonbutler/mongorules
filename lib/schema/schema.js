...
const Schema = require('./schemaClass');
...
self.preprocessSchema = (schema) => {
  ...
  for (let fieldKey in schema) {
    ...
    // Tells us if a field is inside an array of subdocuments. '/' means we are not.
    rootKey = type === 'arrayofobjects' ? fieldKey : fieldKey.indexOf(rootKey) === 0 ? rootKey : '/';

    fieldSchema._isRoot = rootKey === '/';
    fieldSchema._type = type;
    fieldSchema = self._setSchemaFieldDefaults(fieldSchema);
    fieldSchema = self._arrayifySchemaField(fieldSchema);
    self._validateSchemaField(fieldSchema, fieldKey);
    obj[fieldKey] = fieldSchema;
  } // end for loop.
