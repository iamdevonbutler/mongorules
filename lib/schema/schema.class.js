...
module.exports = class Schema {
  ...
  getFieldSchema(key, payloadPath, requireSiblings = true, requireChildren = false, isSetOperation = false, isArrayItemUpdate = false) {
    ...
    keys
      .filter(schemaKey => {
        let fieldSchema = this._fields[schemaKey];
      ...
      .forEach(schemaKey => {
        ...
        if (fieldSchema.default !== undefined) {
          this.addDefault(schemaKey, payloadPath, isArrayItemUpdate);
        }
    });
  ...
  addDefault(key, payloadPath, isArrayItemUpdate) {
    if (this._defaults.indexOf(key) === -1) {
      let isEmbeddedFieldUpdate = payloadPath.length === 1 && payloadPath[0].split('.').length > 1;
      this._defaults.push({key, isEmbeddedFieldUpdate, isArrayItemUpdate});
    }
  }
  ...
  getDefaults(set) {
    return this._defaults
      ...
      .map(item => {
        ...
        var {key, isEmbeddedFieldUpdate, isArrayItemUpdate} = item;

        // @todo hack...
        // This sibling business - documents in arrays have relative and not absolute keys.
        sibling = this._getSibling(key, set);
        if (sibling) {
          payloadPath = sibling.payloadPath
            .reduce((prev, current) => prev.concat(current.split('.')), []) // normalize path so we can slice off the end key.
            .slice(0, -1);
          payloadPath.push(key.split('.').slice(-1).join('.'));
          payloadPath = isEmbeddedFieldUpdate || isArrayItemUpdate ? [payloadPath.join('.')] : payloadPath;
        }
        else {
          payloadPath = [key];
        }
