...
const {preprocessSchema, Types} = require('./schema');
...
self.getDatabase = (connectionName, databaseName) => {
  ...
  getModel = self._getModel.bind(this);
  handler = proxyHandler(connectionName, databaseName, database, getModel);
...
self.addModel = (connectionName, databaseName, collectionName, model) => {
  ...
  models[connectionName][databaseName][collectionName] = {
    schema: model.schema ? preprocessSchema(model.schema) : null,
