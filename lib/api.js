...
class Api {
   ...
   * @tests none.
   */
  close(connectionName, removeModels = true) {
    var connection;
    if (!connectionName) {
    ...
    return new Promise((resolve, reject) => {
      connection.close((err, result) => {
        ...
        delete this._databases[connectionName];
        if (removeModels) {
          delete this._models[connectionName];
        }
   ...
   * @api public.
   */
  removeModels(connectionName, databaseName) {
   ...
   * @api public.
   */
  removeModel(connectionName, databaseName, collectionName) {
