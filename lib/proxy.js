 ...
 * @param {String} methodName
 */
function handleDatabaseMethod(connectionName, databaseName, db, methodName) {
  var method;
  methodName = methodName.slice(1); // names are prefixed w/ a `$`.
  method = db[methodName];
  if (method) {
    let handler = step3(connectionName, databaseName, null, method, methodName, db, null, null, false);
 ...
 */
function step3(connectionName, databaseName, collectionName, method, methodName, ctx, schema, localErrorHandler, validate) {
  return {
    apply(func, that, args) {

      if (validate) {
        let {errors, args: _args} = preprocessQuery(connectionName, databaseName, collectionName, methodName,  args, schema);
        if (errors) {
  ...
  return {
    get(obj, operation) {
      ...
      if (modelMethod) {
        ...
        handler = step3(connectionName, databaseName, collectionName, collectionMethod, operation, collection, schema, localErrorHandler, validate);
      }
  ...
  return {
    get(obj, property) {
      ...
      if (isDatabaseMethod(property)) {
        // operation = property.
        return handleDatabaseMethod(connectionName, databaseName, db, property);
      }

      // Is a call on a collection method or static method...
      // e.g. db.users.find().
      // let collectionName = property.
      let handler = step2(connectionName, databaseName, property /*collectionName*/, db, getModel, validate);
      return new Proxy({}, handler);
    }
  };
};
