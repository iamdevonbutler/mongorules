...
const {preprocessOperation} = require('./preprocess');
...
function handleDatabaseMethod(connectionName, databaseName, db, methodName, globalErrorHandler) {
  ...
  if (method) {
    let handler = step3(connectionName, databaseName, null, method, methodName, db, null, false, null, globalErrorHandler);
 ...
 */
function step3Static(connectionName, databaseName, db, getModel, staticFunction, globalErrorHandler) {
  return {
    apply(func, that, payload) {
      var handler, ctx;
      handler = step1(connectionName, databaseName, db, getModel);
      ctx = new Proxy(db, handler);
      return co(staticFunction.bind(ctx, ...payload)).catch(globalErrorHandler);
    }
  };
}
 ...
 * @param {Function} globalErrorHandler
 */
function step3(connectionName, databaseName, collectionName, method, methodName, ctx, schema, validate, localErrorHandler, globalErrorHandler) {
  return {
    apply(func, that, args) {

      if (validate) {
        let {errors, args: args2} = preprocessOperation(connectionName, databaseName, collectionName, methodName,  args, schema);
        if (errors) {
          handleErrors(errors, 'preprocess', collectionName, methodName, localErrorHandler, globalErrorHandler);
      ...
      return new Promise((resolve, reject) => {
        ...
        args.push((err, result) => {
          if (err) {
            handleErrors(err, null, collectionName, methodName, localErrorHandler, globalErrorHandler);
            reject(err);
          }
          resolve(result)
        });
...
function step2(connectionName, databaseName, collectionName, db, getModel, validate, globalErrorHandler) {
  return {
    get(obj, operation) {
      ...
      if (!model) debug(`Collection "${collectionName}" - no model found (${connectionName}/${databaseName}).`);
      if (model && !schema) debug(`Collection "${collectionName}" - no schema found (${connectionName}/${databaseName}).`);

      validate = shouldpreprocessOperation(operation, validate, schema);

      modelMethod = model && model.methods && model.methods[operation];
      if (modelMethod) {
        handler = step3Static(connectionName, databaseName, db, getModel, modelMethod, globalErrorHandler);
      }
      else {
        ...
        handler = step3(connectionName, databaseName, collectionName, collectionMethod, operation, collection, schema, validate, localErrorHandler, globalErrorHandler);
      }
  ...
  return {
    get(obj, property) {
      ...
      if (isDatabaseMethod(property)) {
        // operation = property.
        return handleDatabaseMethod(connectionName, databaseName, db, property, globalErrorHandler);
      }

      // Is a call on a collection method or static method...
      // e.g. db.users.find().
      // let collectionName = property.
      let handler = step2(connectionName, databaseName, property /*collectionName*/, db, getModel, validate, globalErrorHandler);
      return new Proxy({}, handler);
    }
  };
};
