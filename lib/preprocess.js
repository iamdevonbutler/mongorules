...
self._removeEmptyDocumentsFromPayload = (documents) => {
  return documents.filter((_document) => {
    if (!utils.isType(_document, 'object')) {
      return false;
    }
...
self._preprocessInsert = (documents, schema) => {
  ...
  if (!utils.isType(documents, 'array')) {
    documents = [documents];
  }
...
self._deconstructPayload = (payload, parentKey) => {
  ...
  if (!payload || !utils.isType(payload, 'object')) {
    return {};
  }
  ...
  for (let payloadKey in payload) {
    ...
    valueIsObject = utils.isType(fieldValue, 'object');
    valueIsObject = valueIsObject && !utils.isType(fieldValue, 'date');
    isObjectID = utils.isObjectId(fieldValue);
...
self._getQueryFields = (query, parentKey) => {
  ...
  for (let fieldKey in query) {
    ...
    valueType =  utils.getType(fieldValue);
    valueType = valueType === 'object' && utils.isObjectId(fieldValue) ? 'string' : valueType;
...
self._preprocessArrayOfValues = (fieldKey, fieldObj, fieldSchema, operation) => {
  ...
  if (fieldObj.itemInArrayUpdate || (!fieldObj.isEach && isArrayUpdateOperation)) {
    ...
    // Transform value.
    fieldObj.value = transform.transformValue(fieldObj.value, fieldSchema, 0);
  }
  ...
  for (let i in fieldObj.value) {
    ...
    // Transform value.
    fieldObj.value[i] = transform.transformValue(value, fieldSchema, 0);
  }
...
self._preprocessObject = (obj, fieldKey, schema, operation, isUpsert) => {
  ...
  // Validate object.
  errors = new objectValidator(obj, fieldKey, fieldSchema);

  if (!errors) {
    // Run custom transform function.
    obj = transform.transformFunction(obj, fieldSchema, 0);
    if (utils.isType(obj, 'object')) {
...
self._preprocessPayload = (payload, schema, operation, isUpsert, parentKey) => {
  ...
  for (let payloadKey in payload) {
    ...
    if (schema[fullKey].filterNulls && utils.isType(fieldObj.value, 'array')) {
      fieldObj.value = utils.filterNulls(fieldObj.value);
    }
