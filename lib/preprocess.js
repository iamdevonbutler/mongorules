  ...
  */
  ...
  _preprocessInsert(documents, schema) {
    ...
    for (let i in documents) {
      ...
      parsedPayload = this._deconstructPayload(documents[i]);
  ...
  _preprocessUpdate(payload, schema, query = {}, isUpsert = false) {
    ...
    for (let operation in payload) {
      ...
      // Preprocess document.
      parsedPayload = this._deconstructPayload(payload[operation]);
  ...
  /**
   * Validate and transform a value given a schema and preprocessed payload object.
   * @param {Object} fieldObj - the result of _deconstructPayload() for a field.
   * @param {Object} schema - the fieldSchema for a field.
   * @param {String} [operation] - e.g. $set, $addToSet
   * @return {Errors Array|null}
   * @api private.
   * @tests none.
   */
  _preprocessFieldValue(fieldObj, fieldSchema, operation) {
    var value, fieldKey, errors;

    fieldKey = fieldObj['payloadPath'].join('.');
  ...
  _preprocessPayload(payload, schema, operation) {
    let errors = [];
    // Clone schema because we will be modiffying things.
    schema = _.clone(schema);
    // Itterate over each item in the payload object.
    for (let fieldKey in payload) {
      let fieldSchema, fieldObj, errors;
      fieldSchema = schema[fieldKey];
      fieldObj = payload[fieldKey];
      ...
      switch (fieldSchema._type) {
        case 'value':
          errors = this._preprocessFieldValue(fieldObj, fieldSchema, operation);
          break;
      }
      // Handle errors.
      if (errors) {
