  ...
  */
  ...
  _preprocessQuery(argumentsList, model, action) {
    ...
    switch (action) {
      ...
      case 'update':
        ...
        query = arguments[0];
        isUpsert = argumentsList[2] && argumentsList[2].isUpsert;
      ...
      case 'findAndModify':
        ...
        query = arguments[0];
        isUpsert = argumentsList[3] && argumentsList[3].isUpsert;
        result = this._preprocessUpdate(payload, model.schema, query, isUpsert);
  ...
  _getQueryFields(query, parentKey = '') {
    ...
    for (let fieldKey in query) {
      ...
      let type, fieldValue, isMongoOperator;
      ...
      if (type === 'object') {
        // fieldKey is $elemMatch, but $elemMatch has a parentKey - use the
        // existing parentKey.
        if (fieldKey === '$elemMatch') {
          ...
          fields.push(this._getQueryFields(fieldValue, fieldKey));
        }
      }

      // Add full field path when an $elemMatch operator is used.
      if (parentKey && !isMongoOperator) {
        fields.push(parentKey+'.'+fieldKey);
      }
      else {
  ...
  _queryFieldsExistInSchema(query, schemaFields) {
    ...
    for (let queryField in queryFields) {
  ...
  _preprocessUpdate(payload, schema, query = {}, isUpsert = false) {
    var result, _document, hasOperator, isUpdate;
    ...
    // If this is an upsert, process operation as an insert.
    isUpdate = isUpsert ? false : true;
    ...
    if (!hasOperator) {
      ...
      result = this._preprocessDocument(_document, schema, isUpdate);
