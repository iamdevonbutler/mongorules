...
module.exports = {
  ...
  _deconstructPayload(payload, parentKey) {
    ...
    for (let payloadKey in payload) {
      ...
      isObjectID = valueIsObject ? fieldValue._bsontype && fieldValue.id : false;
   ...
   * @tests none.
   */
  _preprocessArrayOfObjects(fieldObj, fieldKey, schema, operation, isUpsert) {
    ...
    for (let payloadKey in payload) {
      ...
      switch (schema[fullKey]._type) {
        ...
        case 'arrayofvalues':
          ...
          break;
        case 'arrayofobjects':
          err = this._preprocessArrayOfObjects(fieldObj, fullKey, schema, operation, isUpsert);
          break;
      }
      // Handle errors.
      if (err) {
  ...
  */
   ...
   * @tests unit.
   */
  _filterSchema(payload, schema, operation, isUpsert, parentKey) {
    let arrayFields = [];
