...
module.exports = {
  ...
  _deconstructPayload(payload) {
    ...
    for (let payloadKey in payload) {
      ...
      if (valueIsObject) {
        let result, documentKey, modifiers, fullKey;
        modifiers = this._spliceModifiers(fieldValue);
        result = this._deconstructPayload(fieldValue, fullKey);
  ...
  _validateRequiredFields(payload, schema, operation, parentKey) {
    ...
    for (let schemaKey in schema) {
      ...
      if (schema[schemaKey].required && _.isUndefined(payload[payloadKey])) {
        ...
        if (!operation) {
          errors.push('required '+ schemaKey);
          continue;
        }
        ...
        for (let payloadKey in payload) {
          ...
          payloadKey = parentKey ?  parentKey + '.' + payloadKey : payloadKey;
