 ...
 * @tests intergration
 */
self._preprocessQuery = (args, schema, operation) => {
 ...
 * @tests none.
 */
self._preprocessArrayOfValues = (fieldObj, fieldKey, schema, operation) => {
 ...
 * @tests none.
 */
self._preprocessArrayOfObjects = (fieldObj, fieldKey, schema, operation, isUpsert, parentKey) => {
 ...
 * @tests none.
 */
self._preprocessFieldValue = (fieldObj, fieldKey, schema) => {
  var value, errors;
...
self._preprocessPayload = (payload, schema, operation, isUpsert, parentKey) => {
  ...
  for (let payloadKey in payload) {
    ...
    switch (schema[fullKey]._type) {
      case 'value':
        err = self._preprocessFieldValue(fieldObj, fullKey, schema);
        break;
      case 'arrayofvalues':
        err = self._preprocessArrayOfValues(fieldObj, fullKey, schema, operation);
        break;
      case 'arrayofobjects':
        err = self._preprocessArrayOfObjects(fieldObj, fullKey, schema, operation, isUpsert, parentKey);
        break;
    }
    // Handle errors.
    if (err && err.length) {
