...
module.exports = {
  ...
  */
  ...
  /**
   * Given an update payload, remove embedded fields from payload (by
   * reference), remove fields that DNE in schema (by reference),
   * and return them in a new object.
   ...
   * @tests none.
   */
  _spliceEmbeddedFieldsFromPayload(payload, schemaFields) {
    var topLevelProperties,
      embeddedFields = {};

    topLevelProperties = Object.keys(payload);
    for (property of topLevelProperties) {
      let isEmbeddedField, strippedKey, inSchema;
      strippedKey = fieldKey.replace('$.', '').replace(/\.\d/g, '');
      isEmbeddedField = strippedKey.indexOf('.') > -1;
      if (isEmbeddedField) {
        inSchema = schemaFields.indexOf(strippedKey) > -1;
        if (inSchema) {
          // Add property to embeddedFields object.
          embeddedFields[property] = payload[property];
        }
    ...
    return Object.keys(embeddedFields).length ? embeddedFields : null;
  },

  /**
   * Validate and transform em
   */
  _preprocessEmbeddedFields(embeddedFields, schema) {
    var _document = {},
      documentKeys,
      result;

    // Build document.
    _document = this._getDocumentFromPayload(embeddedFields, operation);
    preprocessedDocument = this._preprocessDocument(_document, schema, isUpsert);

    // documentKeys = Object.keys(result.document).length;
    // if (!documentKeys || result.errors) {
    //   return {
    //     payload: embeddedFields,
    //     errors: result.errors || ['Empty document post . Nothing to update.']
    //   };
    // }

    // for (let fieldKey in embeddedFields) {
    //   let strippedKey, fieldValue;
    //   strippedKey = fieldKey.replace('$.', '').replace(/\.\d/g, '');
    //   fieldValue = __.deepGet(result.document, strippedKey);
    //   embeddedFields[fieldKey] = fieldValue;
    // }
  ...
  _preprocessUpdate(payload, schema, query = {}, isUpsert = false) {
    ...
    for (let operation in payload) {
      ...
      // Preprocess embedded fields separately.
      embeddedFields = this._spliceEmbeddedFieldsFromPayload(payload[operation], schema._fields);
      if (embeddedFields) {
        preprocessedEmbeddedFields = this._preprocessEmbeddedFields(embeddedFields, schema);
        if (preprocessedEmbeddedFields.errors) {
          errors.push(preprocessedEmbeddedFields.errors);
