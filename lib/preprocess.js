  ...
  */
  ...
  _preprocessUpdate(payload, schema) {
    ...
    for (let operation in payload) {
      switch (operation) {
        ...
        case '$mul':
        case '$rename':
          throw new Error('Update operator "$rename" is not supported by monogproxy.');
          break;
        case '$setOnInsert':
          throw new Error('Update operator "$setOnInsert" is not supported by monogproxy.');
          break;
        case '$set':
          ...
          break;
        case '$unset':
          throw new Error('Update operator "$unset" is not supported by monogproxy.');
          break;
        case '$min':
        case '$max':
        case '$currentDate':
          throw new Error('Update operator "$currentDate" is not yet supported by monogproxy.');
          break;
        case '$addToSet':
        case '$pop':
      ...
      if (result && result.document && !result.errors) {
  ...
  _replaceDocumentInPayload(obj, _document) {
    for (let field in obj) {
      ...
      fieldKey = field.replace('$.', '');
      if (utils._isType(fieldValue, 'object')) {
        let documentValue = __.deepGet(_document, fieldKey);
        if (fieldValue.$each) {
          obj[field].$each = documentValue;
        }
        else {
          obj[field] = this._replaceDocumentInPayload(fieldValue, documentValue);
        }
      }
      else {
