  ...
  _preprocessUpdate(payload, schema) {
    ...
    if (!hasOperator) {
      ...
      if (!Object.keys(result.document).length) {
        return { payload: payload, errors: errors ? errors : ['Empty document. Nothing to update.'] };
      }
      if (!result.errors) {
    ...
    for (let operation in payload) {
      if (validOperations.indexOf(operation) > -1) {
        ...
        result = this._preprocessDocument(_document, schema, true);
        if (!Object.keys(result.document).length) {
          return { payload: payload, errors: errors ? errors : ['Empty document post . Nothing to update.'] };
        }
      ...
      if (invalidOperations.indexOf(operation) > -1) {
        throw new Error(`Update operator "${operation}" is not supported by monogproxy. Use the 'novalidate' prefix to continue operation.`);
      }
    }
          ...
          case 'arrayObjects':
            ...
            fieldValue = fieldValue.map((obj) => {
              ...
              if (!utils._isType(obj, 'object')) {
                throw new Error('Custom transformations on field "'+fieldKey+'" must return an Object');
              }
          ...
          case 'arrayArrayValues':
            ...
            fieldValue = fieldValue.map((value) => {
              ...
              if (!utils._isType(value, 'array')) {
                throw new Error('Custom transformations on field "'+fieldKey+'" must return an Array');
              }
