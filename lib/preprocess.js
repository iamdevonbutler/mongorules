...
module.exports = {
  ...
  */
  ...
  _preprocessUpdate(payload, schema, query = {}, isUpsert = false) {
    ...
    // var insupportedOperations = ['$rename', '$setOnInsert', '$unset', '$currentDate', '$pop', '$pullAll', '$pull', '$pushAll'];
    ...
    for (let operation in payload) {
      ...
      // Preprocess document.
      _document = this._preprocessPayload(payload[operation], operation);
   ...
   * @tests unit.
   */
  _preprocessPayload(payload, operation = null) {
    var _document = {};
    ...
    for (let payloadKey in payload) {
      ...
      if (valueIsObject) {
        ...
        result = this._preprocessPayload(fieldValue, operation);
        for (let field in result) {
          ...
          _document[documentKey] = {
            operation: result[field]['operation'],
