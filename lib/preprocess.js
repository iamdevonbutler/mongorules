...
module.exports = {
  ...
  _preprocessArrayOfObjects(fieldObj, fieldKey, schema, operation) {
    ...
    for (let i in fieldValue) {
      ...
      // If there is a payload, preprocess the contents of the object.
  ...
  _validateRequiredFields(payload, schema, operation, parentKey) {
    ...
    for (let schemaKey in schema) {
      let payloadKey;
      payloadKey = parentKey ? schemaKey.slice(parentKey.length + 1) : schemaKey;
      // Only process fields required in schema that do not exist in payload.
      if (schema[schemaKey].required && _.isUndefined(payload[payloadKey])) {
        let err;
  ...
  */
  _setDefaultValues(payload, schema, operation, parentKey) {
    ...
    // Itterate over each schema key.
    for (let schemaKey in schema) {
      let fieldSchema, relativeSchemaKey, isSubdocumentField = true;
      ...
      if (payloadKeys.indexOf(relativeSchemaKey) === -1 && !_.isUndefined(fieldSchema.default)) {

        // If this is an update, only set a default if a subdocument is being updated.
        if (operation) {
          // Check payload to see if it contains sibling fields - if it does,
          for (let payloadKey in payload) {
            let fullKey;
            ...
            fullKey = parentKey ?  parentKey + '.' + payloadKey : payloadKey;
            if (schemaKey.split('.')[0] === fullKey.split('.')[0]) {
              isSubdocumentField = payload[payloadKey].fieldInSubdocument;
              break;
            }
          } // end for of loop.
        } // end if

        if (isSubdocumentField) {
          // Set default value.
