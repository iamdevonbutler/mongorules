  ...
  */
  ...
  _preprocessInsert(documents, schema) {
    ...
    for (let i in documents) {
      ...
      result = this._preprocessPayload(parsedPayload, _.clone(schema));
      if (result.errors) {
        errors = result.errors;
        break;
      }
  ...
  _preprocessUpdate(payload, schema, query = {}, isUpsert = false) {
    ...
    for (let operation in payload) {
      ...
      result = this._preprocessPayload(parsedPayload, _.clone(schema), operation);
  ...
  _preprocessArrayOfObjects(fieldObj, fieldKey, schema, operation) {
    var value, fieldSchema, errors, result;
    ...
    for (let obj of fieldObj.value) {
      ...
      obj = this._deconstructPayload(obj);
  ...
  _preprocessPayload(payload, schema, operation, parentKey = '') {
    ...
    for (let payloadKey in payload) {
      ...
      // Remove item from schema after processing.
      delete schema[fullKey];
    }
