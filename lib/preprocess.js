  ...
  */
  ...
  _replaceDocumentInPayload(obj, _document, operation) {
    ...
    for (let field in obj) {
      ...
      if (utils._isType(fieldValue, 'object') && !isArrayItem) {
        ...
        // item, e.g. field.1, a scalar would previously be converted to an
        //  array for validation, if so, change it back. @hack.
  ...
  _getDocumentFromPayload(obj, operation) {
    ...
    for (let field in obj) {
      ...
      // If given a scalar for array operations, convert to array just for validation
      // value conveted back to original type in _replaceDocumentInPayload.
      // Do the same @hack for queries w/ field.$ or field.1, that operate on
      // array items.
      if (['$addToSet', '$push'].indexOf(operation) > -1 || isArrayItem) {
