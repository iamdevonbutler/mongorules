...
module.exports = {
  ...
  _preprocessInsert(documents, schema, parentKey = '') {
    ...
    innerArrayValidator = new InnerArrayValidator();
      ...
      for (let schemaType in schema) {
        ...
        for (let fieldKey in schema[schemaType]) {
          ...
          if (fieldSchema.filterNulls && schemaType !== 'values') {
            fieldValue = utils.filterNulls(fieldValue);
          }
          ...
          switch (schemaType) {
            ...
            case 'arrayObjects':
              ...
              // Call custom validate fuction.
              result = ArrayValidator.validateFunction(fieldValue, fieldSchema);
              if (!result) {
                errors.push({ field: fieldKey, property: 'validate', value: fieldValue });
                break;
              }
            ...
            case 'arrayArrayValues':
              ...
              // Array must contain at least one item, that will later be validate to
              // ensure it is an array.
              if (fieldSchema.required && !fieldValue.length) {
                errors.push({ field: fieldKey, property: 'required', value: fieldValue });
                break;
              }
              ...
              fieldValue = fieldValue.map((value) => {
                ...
                // Itterate over each value in the mested array.
                return value.map((val) => {
                  result = valueValidator.validate(val, fieldKey, fieldSchema);
                  if (!result) {
                    errors.push(valueValidator.getErrors());
                    return;
                  }
