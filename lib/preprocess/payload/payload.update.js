...
module.exports = class UpdatePayload extends Payload {
  ...
  preprocessSet(payload, schema, cacheKey) {
    var errors = [], processAsSetOperation, payloadKeys;
    processAsSetOperation = !this.isUpsert;
    payload = this.deconstructPayload(payload);

    // @todo hack - process all sets as embedded field updates.
    payloadKeys = Object.keys(payload);
    payloadKeys.forEach(key => {
      payload[key].payloadPath = [payload[key].payloadPath.join('.')];
    });

    errors = this.buildPayloadSet(payload, schema, processAsSetOperation);
    ...
    payloadKeys.forEach(key => {
      ...
      value.forEach((value2, i) => {
        ...
        if (isObject) {
          ...
          obj = new SubdocumentPreprocessor(schema2, schemaKey);
          obj.addPayload([value2]);
          errors2 = obj.preprocess(null, schemaKey);
          if (errors2 && errors2.length) {
            obj.resetPayload();
            errors = errors.concat(errors2);
            return;
    ...
    keys.forEach((operation) => {
      ...
      switch (operation) {
        case '$set':
          ...
          break;
        case '$inc':
          obj[operation] = this.preprocessFieldUpdate(payload[operation], schema._fields);
          break;
        case '$mul':
          obj[operation] = this.preprocessFieldUpdate(payload[operation], schema._fields);
          break;
        ...
        case '$max':
          ...
          break;
        case '$addToSet':
          obj[operation] = this.preprocessArrayUpdate(payload[operation], schema._fields);
          break;
        case '$push':
          obj[operation] = this.preprocessArrayUpdate(payload[operation], schema._fields);
          break;
