...
module.exports = class PayloadItem {
  ...
  preprocess(cacheKey) {
    var fieldSchema, type, schemaKey, value, errors;
    ...
    switch (type) {
      case 'value':
        errors = this.preprocessFieldValue(value, schemaKey, fieldSchema);
        break;
      case 'arrayofvalues':
        errors = this.preprocessArrayOfValues(value, schemaKey, fieldSchema);
        break;
      case 'arrayofobjects':
        ({errors, value} = this.preprocessArrayOfObjects(value, schemaKey, fieldSchema, cacheKey));
        this.value = value;
        break;
    }
  ...
  preprocessArrayOfObjects(fieldValue, fieldKey, fieldSchema, cacheKey) {
    ...
    if (errors && errors.length) {
      return { errors, value: fieldValue };
    }
