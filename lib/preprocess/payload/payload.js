...
module.exports = class Payload {
  ...
  deconstructPayload(payload, parentKey) {
    ...
    for (let payloadKey in payload) {
      let valueIsObject, fieldValue, fieldKey, itemInArray, hasKeys, payloadPath;
  ...
  buildPayloadSet(payload, schema) {
    ...
    keys.forEach((key) => {
      ...
      var {payloadPath, value} = payload[key];
      var embeddedFieldUpdate = false;
      fieldSchema = schema.getFieldSchema(key, embeddedFieldUpdate);
      if (fieldSchema) {
