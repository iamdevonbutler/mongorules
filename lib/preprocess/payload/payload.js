...
module.exports = class Payload {
  ...
  buildPayloadSet(payload, schema) {
    ...
    keys.forEach((key) => {
      ...
      var {payloadPath, value, isEach, itemInArray, embeddedFieldUpdate} = payload[key];
      fieldSchema = schema.get(key, embeddedFieldUpdate);
      if (fieldSchema) {
        payloadItem = new PayloadItem(payloadPath, key, value, isEach, itemInArray, embeddedFieldUpdate, fieldSchema, schema._schema);
