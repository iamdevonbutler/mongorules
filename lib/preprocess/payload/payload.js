...
module.exports = class Payload {
  ...
  deconstructPayload(payload, parentKey) {
    ...
    for (let payloadKey in payload) {
      ...
      if (hasKeys && valueIsObject && !isObjectId(fieldValue)) {
        ...
        for (let field in result) {
          ...
          obj[documentKey] = {
            ...
            isArrayItemUpdate: isArrayItemUpdate || result[field].isArrayItemUpdate,
          };
        }
      }
      else {
   ...
   */
  buildPayloadSet(payload, schema, isSetOperation = false, processAsInsert = true) {
    ...
    keys.forEach((key) => {
      ...
      requireSiblings = !isSetOperation || isArrayItemUpdate;
      requireChildren = isSetOperation && isType(value, 'object');

      fieldSchema = schema.getFieldSchema(key, payloadPath, requireSiblings, requireChildren, isSetOperation, isArrayItemUpdate);
      if (fieldSchema) {
        if (isArrayItemUpdate) {
          payloadItem = new ArrayUpdatePayloadItem(payloadPath, key, value, fieldSchema, schema._fields);
        }
        else {
          payloadItem = new PayloadItem(payloadPath, key, value, fieldSchema, schema._fields);
        }
