...
module.exports = class Payload {
  ...
  deconstructPayload(payload, parentKey) {
    ...
    for (let payloadKey in payload) {
      ...
      if (hasKeys && valueIsObject && !isObjectId(fieldValue)) { // @doc:1
   ...
   */
  buildPayloadSet(payload, schema, isSetOperation = false) {
    ...
    keys.forEach((key) => {
      ...
      fieldSchema = schema.getFieldSchema(key, requireSiblings, requireChildren, isSetOperation);
      if (fieldSchema) {
