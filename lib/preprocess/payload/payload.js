...
module.exports = class Payload {
  ...
  buildPayloadSet(payload, schema) {
    ...
    // @todo this catches requireds but does not set defaults.
    if (!keys || !keys.length) {
      let fieldKeys = schema.getTopKeys();
      errors = fieldKeys.map(key => {
        return schema._schema[key].required ? `Field "${key}" is required in payload ` : null
      }).filter(Boolean);
