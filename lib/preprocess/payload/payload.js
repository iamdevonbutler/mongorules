...
/**
 * @file base class. Contains generic methods
 * related to parsing payloads, building payload sets that
 * become cached, determining which fields are required and
 * when we need to set defaults for each operation type.
 */
...
module.exports = class Payload {
   ...
   *   @param {Mixed} value
   *   @param {Boolean} isArrayItemUpdate
   */
  ...
  buildPayloadSet(payload, schema, subdocumentInArray = false, isSetOperation = false, isUpsert = false) {
    ...
    keys.forEach((key) => {
      ...
      fieldSchema = schema.getFieldSchema(key, valueIsObject);
      if (fieldSchema) {
