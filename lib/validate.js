...
class Validator {
   ...
   */
  _validateRequired() {
    if (!this.schema.required || this._done) {
      return this;
    }
  ...
  validateType(type) {
    ...
    var expected = this.schema.dateFormat ?
      type + ': ' + this.schema.dateFormat : type;
    if (type === 'date') {
      if (!utils._validateDate(this.value, this.schema.dateFormat)) {
        this.addError('date', { expected: expected });
      }
    }
    else {
   ...
   */
  validateDenyXSS() {
    if (!this.schema.denyXSS || this._done) {
      return this;
    }
  ...
  validateFunction(pos = 0) {
    if (this._done || !this.schema.validate[pos]) {
      return this;
    }
    if (!this.schema.validate[pos].call(this.value, this.schema)) {
