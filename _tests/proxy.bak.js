...
module.exports = _this = {
  ...
  _mongodbApplyHandler: {
    apply(target, thisArgument, argumentsList) {
      var state, collection, model, preprocessOperation,
        globalErrorHandler, localErrorHandler, database;
      ...
      database = mongorules.getDatabase();
      if (!database) throw '>>> call the addDatabase() method before operation.';

      collection = database.collection(state.collectionName);
    ...
    get(target, propertyKey, receiver) {
      ...
      // a mongo query method or static model method..
        ...
        if (model && model.methods && model.methods[propertyKey]) {
          ...
          state = _this._cloneState();
          _this._clearState();
          ctx = new Proxy(mongorules, _this._getHandler);
